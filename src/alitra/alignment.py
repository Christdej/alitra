...
from alitra.transformations import transform_position
...
def align_positions(
    ...
    :param rot_axes: Axis of rotation. For rotations in the xy plane (most common),
        this is set to 'z'
    :param rsmd_treshold: The root mean square distance treshold, for the coordinate
    ...
    try:
        _check_rsme_treshold(transform, positions_to, positions_from, rsmd_treshold)
    except Exception as e:
        raise ValueError(e)
...
def _check_rsme_treshold(
    ...
    positions_to_new = transform_position(
        transform, positions_to, from_=positions_to.frame, to_=positions_from.frame
    )
