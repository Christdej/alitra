...
from alitra import transform_orientation, transform_pose, transform_position
...
@pytest.mark.parametrize(
    "euler_array, frame, translation, expected_position",
    [
        (
            np.array([0, 0, 0]),
...
def test_transform_position(default_transform, robot_frame, asset_frame):
    ...
    pos_to = transform_position(
        transform=default_transform, positions=pos, from_=robot_frame, to_=asset_frame
    )
...
def test_transform_pose(
    ...
    expected_pose: Pose = Pose(
        ...
        orientation=Orientation(rotation=default_rotation, frame=asset_frame),
        frame=asset_frame,
    )
